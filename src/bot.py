...
if user_ids_env:
    allowed_user_ids = set(int(uid.strip()) for uid in user_ids_env.split(',') if uid.strip())
else:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    welcome_message = _("¡Bienvenido!\n\nEste bot le ayudará a mejorar y profundizar en ePubs usando <b>Inteligencia Artificial</b>.")
    ...
    if user.id not in allowed_user_ids:
        await update.message.reply_text(_("Lo siento, no tienes permiso para usar este bot. Si crees que esto es un error, contacta con el administrador."))
        return ConversationHandler.END
        ...
        if last_run_str:
            ...
            if time_since_last_run < timedelta(hours=1):
                ...
                await update.message.reply_text(
                    trans("Por favor, inténtelo de nuevo dentro de {} minutos. Así puedo controlar mejor los gastos, ya que es gratuito.").format(minutes)
                )
...
async def language_selected(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if old_lang_selected and old_lang_selected != lang_code:
        ...
        welcome_message = _("¡Bienvenido!\n\nEste bot le ayudará a mejorar y profundizar en la preparación de ePubs usando <b>Inteligencia Artificial</b>.")
...
async def ask_backup(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if update.message:
        await update.message.reply_text(trans("¿Deseas proporcionar tu archivo de respaldo .epublibrary? Por favor envíalo ahora. Si prefieres no hacerlo, escribe 'no'."))
    elif update.callback_query:
        await update.callback_query.message.reply_text(trans("¿Deseas proporcionar tu archivo de respaldo .epublibrary? Por favor envíalo ahora. Si prefieres no hacerlo, escribe 'no'."))
...
async def receive_backup_file_document(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if file_path.endswith(".epublibrary"):
        # Save the file
        file = await context.bot.get_file(file.file_id)
        os.makedirs('userBackups', exist_ok=True)
        await file.download_to_drive('userBackups/{0}.epublibrary'.format(user.id))
        context.user_data['backup_provided'] = True
        # Run describe_epublibrary and post output
        try:
            notesN, inputN, tagMaptN, tagN, bookmarkN, lastModified, userMarkN = core_worker.describe_epublibrary(user.id)
            await update.message.reply_html(trans("""Estado de tu archivo <code>.epublibrary</code>:
<u>Notas:</u> {0}
<u>Tags:</u> {1}
<u>Notas:</u> {2}
<u>Escritos:</u> {3}
...
<u>Última vez modificado:</u> {6}""").format(notesN, tagN, tagMaptN, inputN, bookmarkN, userMarkN, lastModified))
            ...
            logger.error("Error in describe_epublibrary: %s", e)
        ...
        await update.message.reply_text(trans("Formato de archivo erróneo. Por favor, envía un archivo .epublibrary válido."))
        # Ask again
        return RECEIVE_BACKUP_FILE
...
def fetch_url_from_date(date_selection, langSelected):
    try:
        ...
        # Path to the local log database
        log_db_path = 'dbs/log.db'
        need_download = True  # Flag to determine if we need to download the log

        # Check if the log.db file exists
        if os.path.exists(log_db_path):
            # Get the last modified time of the file
            mtime = os.path.getmtime(log_db_path)
            ...
            if datetime.now() - last_modified_date < timedelta(days=30):
                ...
                logger.info("NEED_DOWNLOAD log.db FALSE - Last modified date: {0}".format(last_modified_date))

        if need_download:
            logger.info("NEED_DOWNLOAD log.db TRUE")
            # Proceed to download the latest log.db.gz
            jsonurl = requests.get("https://place.holder/json")
            manifest_id = jsonurl.json()['current']
            log_url = f"https://place.holder/{manifest_id}/gz"
            log_response = requests.get(log_url)

            # Ensure the dbs directory exists
            os.makedirs('dbs', exist_ok=True)

            # Save the downloaded log.db.gz file
            with open('log.db.gz', 'wb') as f:
                f.write(log_response.content)

            # Extract log.db from the gzipped file
            with gzip.open("log.db.gz", "rb") as gz_file:
                with open(log_db_path, 'wb') as db_file:
        ...
        # Map the language codes to the ones used by me personally
        lang_codes = {
            'es': 'S',          # Español
            'en': 'E',          # English
            'fr': 'F',          # Français
            'pt-B': 'OLA',     # Português (Portugal)
            'pt-A': 'OOO',     # Português (Brasil)
            'de': 'X',          # Deutsch
            'bg': 'BL',         # Български
            'it': 'I',          # Italiano
            'nl': 'K',          # Nederlands
        ...
        magazine_url = f"https://www.place.holder/{lang}{year}{month}{symbol}"
        magazine = requests.get(magazine_url).text
        soup = BeautifulSoup(magazine, features="html.parser")
        div_articles = soup.find_all("div", {"class": "lass-40"})

        # Get the URL of the specific study article based on the date
        url = "https://www.place.holder" + div_articles[delta_start_week_found].find("a").get("href")
...
async def receive_date_or_url_choice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_choice == 'date':
        ...
        await query.edit_message_text(trans("Por favor, envía la URL del ePub que deseas preparar."))
        return RECEIVE_URL
...
def get_default_questions(trans):
    init_question_1 = trans("Una ilustración o ejemplo para explicar algún punto principal")
    init_question_2 = trans("Una experiencia en concreto, aportando referencias exactas de place.holder, que esté muy relacionada")
    init_question_3 = trans("Una explicación sobre uno de las referencias que aparezcan, que aplique.")
...
async def epub_prepare(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    logger.info("BEGIN EPUB_PREPARE - User ID: {0} - URL: {1} - WeekDelta: {2} - Questions: {3} - LastRun: {4}".format(user.id, url, date, qs, lastRun))
    ...
    if any(qs):
        ...
        if (url is not None):
            ...
            try:
                ...
                filenameepub, filenamedoc, filenamepdf = core_worker.main(url, user.id, qs, langSelected)

                if context.user_data.get('backup_provided', False):
                    await message.reply_text(trans("Aquí tienes tu fichero, impórtalo."))
                else:
                    await message.reply_text(trans("Aquí tienes tu fichero, impórtalo\nNota: Al no haber proporcionado tu copia de seguridad, hazlo con precaución, puedes perder tus datos de la app."))

                # Send the Epub Library file
                await message.reply_document(document=open(filenameepub, "rb"))
                os.remove(filenameepub)
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            EPUB_PREPARE: [
                MessageHandler(filters.StatusUpdate.ALL, epub_prepare),
                CommandHandler('cancel', cancel),
            ],
            AFTER_PREPARATION: [
