...
SESSION_NAME: str = "askepub_testing"
...
async def run_test(client: Client):
    controller = BotController(
        peer="@askepub_dev_bot", # @askepub_bot
        client=client,
    ...
    async with controller.collect(count=2) as response:
        await controller.send_command("url_select", args=["https://www.place.holder/html"])
    ...
    assert "place.holder" in response.messages[0].text
    print("Url selected")
    ...
    async with controller.collect(count=1) as response:
        await controller.send_document(str(Path(__file__).parent.resolve()) + os.sep + "test.epublibrary")
    ...
    async with controller.collect(count=9, max_wait=600) as response:
        await controller.send_command("epub_prepare")
    ...
    assert "Library" in response.messages[4].text
    assert response.messages[5].document.file_name.endswith(".epublibrary")
